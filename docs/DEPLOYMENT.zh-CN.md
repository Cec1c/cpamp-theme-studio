...
5. 已安装 bootstrap 时，插件目录写入会自动触发 broker；等待市场流程完成并让 CPA 恢复即可。Theme Studio 三点菜单中位于“重新安装”上方的“重新启动”项仍可用于主动重启，并复用同一个 broker。未安装 bootstrap 时，使用已配置的旧重启方式或手工重启实际 CPA 服务。主题工作室的面板 watcher 属于进程内状态；CPA 热重载可能让退役版本存活到进程退出，导致新旧 loader 缓存版本交替写入。
...
“重新启动”菜单项不会调用公开重启 API，也不会读取 Management Key。它生成随机的一次性 `restart_request`，打开 CPAMP 原生“编辑配置”抽屉，再通过当前已认证的管理会话保存该值；插件会留出保存响应返回的时间后再安排重启。
...
- 浏览器开发者工具没有主题插件报错。
- Theme Studio 原生三点菜单在“重新安装”上方出现且仅出现一个“重新启动”项，并复用相同的结构、样式与图标；取消二次确认不会改变 CPA 进程或配置。
